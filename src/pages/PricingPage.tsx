              ...
              <div className="bg-primary/5 border-b p-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-semibold">
                    {currentLanguage === 'sr' ? 'Naše Usluge' : 'Our Services'}
                  </h2>
                </div>
              </div>
              <div className="p-6">
              ...
              <div className="p-6">
                <div className="space-y-4 text-base leading-relaxed">
                  ...
                  <h3 className="font-semibold text-lg mt-6 mb-3">
                    {currentLanguage === 'sr' ? 'Posredovanje u poslovima zakupa nepokretnosti' : 'Brokerage in real estate leasing'}
                  </h3>
                      ...
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-2 text-left font-medium text-gray-500">
                            {currentLanguage === 'sr' ? 'Iznos mesečne zakupnine' : 'Monthly Rent Range'}
                          </th>
                          <th className="px-4 py-2 text-left font-medium text-gray-500">
                            {currentLanguage === 'sr' ? 'Iznos posredničke nadoknade' : 'Broker\'s Fee'}
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y">
                        <tr>
                          <td className="px-4 py-2">
                            {currentLanguage === 'sr' ? 'od 550 do 1200 eur-a' : 'from € 550 to € 1200'}
                          </td>
                          <td className="px-4 py-2">
                            {currentLanguage === 'sr' ? '50% od iznosa mesečnog zakupa' : '50% of the monthly rent'}
                          </td>
                        </tr>
                        <tr>
                          <td className="px-4 py-2">
                            {currentLanguage === 'sr' ? 'od 1250 eur-a i više' : 'from € 1250 and more'}
                          </td>
                          <td className="px-4 py-2">
                            {currentLanguage === 'sr' ? '100% od iznosa mesečnog zakupa' : '100% of the monthly rent'}
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-2 text-left font-medium text-gray-500">
                            {currentLanguage === 'sr' ? 'Period zakupa' : 'Lease Period'}
                          </th>
                          <th className="px-4 py-2 text-left font-medium text-gray-500">
                            {currentLanguage === 'sr' ? 'Posrednička nadoknada' : 'Broker\'s Fee'}
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y">
                        <tr>
                          <td className="px-4 py-2">
                            {currentLanguage === 'sr' ? 'od 13 do 20 meseci' : 'from 13 to 20 months'}
                          </td>
                          <td className="px-4 py-2">70%</td>
                        </tr>
                        <tr>
                          <td className="px-4 py-2">
                            {currentLanguage === 'sr' ? '23 meseca i duže' : '23 months and longer'}
                          </td>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 py-2 text-left font-medium text-gray-500">
                          {currentLanguage === 'sr' ? 'Tip nepokretnosti' : 'Property Type'}
                        </th>
                        <th className="px-4 py-2 text-left font-medium text-gray-500">
                          {currentLanguage === 'sr' ? 'Cena (u din. protivvrednosti)' : 'Price (in RSD equivalent)'}
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      <tr>
                        <td className="px-4 py-2">
                          {currentLanguage === 'sr' ? 'Stan, kuća od 20 do 110 m²' : 'Apartment, house from 20 to 110 m²'}
                        </td>
                        <td className="px-4 py-2">€ 150</td>
                      </tr>
                      <tr>
                        <td className="px-4 py-2">
                          {currentLanguage === 'sr' ? 'Stan, kuća više od 110 m²' : 'Apartment, house more than 110 m²'}
                        </td>
                        <td className="px-4 py-2">€ 250</td>
                      </tr>
                      <tr>
                        <td className="px-4 py-2">
                          {currentLanguage === 'sr' ? 'Poslovni prostor - po upitu' : 'Business space - upon request'}
                        </td>
                        <td className="px-4 py-2">
                          {currentLanguage === 'sr' ? 'od € 250 do € 1000' : 'from € 250 to € 1000'}
                        </td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div className="border-0 shadow-lg rounded-lg bg-white">
              <div className="bg-primary/5 border-b p-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-semibold">
                    {currentLanguage === 'sr' ? 'Menadžment nepokretnosti' : 'Real estate management'}
                  </h2>
                </div>
                <p className="text-base mt-2 text-muted-foreground">
                  {currentLanguage === 'sr' ? 'Cena usluge mesečno' : 'Price of service monthly'}
                </p>
              </div>
              <div className="p-6">
                <div className="overflow-x-auto">
